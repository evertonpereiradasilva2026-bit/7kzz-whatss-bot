...
import {get,set,toggleArray,save,getSecurity,setSecurity} from './store.js';
import {mainMenu,submenu,groups} from './menu.js';
...
const arg=(a,n=0)=>a.slice(n).join(' ');
const mention=m=>{let c=m.message?.extendedTextMessage?.contextInfo?.mentionedJid||[];return c};
...
function addWarn(chat,user,reason){
 const key=chat+'|'+jidNum(user);
 const n=(warns.get(key)||0)+1;
 warns.set(key,n);
 return n;
}

function clearWarn(chat,user){
 warns.delete(chat+'|'+jidNum(user))
}

async function punish(s,chat,user,reason,security){
 const n=addWarn(chat,user,reason);
 const action=security.action||'delete';
 let msg=`⚠️ Proteção 7kzz\nMotivo: ${reason}\nAdvertência: ${n}/${security.warnLimit}`;
 
 if(action==='delete'||n<security.warnLimit){
  try{await s.sendMessage(chat,{delete:m.key})}catch{}
  if(action==='warn'||n<security.warnLimit)return {n,done:false,msg};
 }
 
 if(action==='kick'||(n>=security.warnLimit&&action==='delete')){
  try{await s.groupParticipantsUpdate(chat,[user],'remove')}catch{}
  return {n,done:true,msg:msg+'\nAção: remoção.'};
 }
 
 return {n,done:false,msg};
}

async function start(){
 const {state,saveCreds}=await useMultiFileAuthState('./auth');
 const {version}=await fetchLatestBaileysVersion();
 const s=makeWASocket({version,auth:state,logger:log,browser:['7kzz','Chrome','2.0']});
 
 s.ev.on('creds.update',saveCreds);
 
 s.ev.on('connection.update',u=>{
  if(u.qr){console.log('QR:');qr.generate(u.qr,{small:true})}
  if(u.connection==='open')console.log('7kzz online');
  if(u.connection==='close'&&u.lastDisconnect?.error?.output?.statusCode!==DisconnectReason.loggedOut)start()
 });

 s.ev.on('groups.upsert',gs=>{
  for(const g of gs){
   get().connectedGroups[g.id]=g.subject
  }
  save()
 });
 ...
 s.ev.on('messages.upsert',async({messages})=>{
  ...
  const raw=t.slice(p.length).trim();
  const parts=raw.split(/\s+/);
  const c=parts.shift().toLowerCase();
  const a=parts;
  const user=m.pushName||jidNum(m.key.participant||chat)||'user';
  ...
  try{
   ...
   if(chat.endsWith('@g.us') && !isOwner(m)){
    ...
    if(sec.antispam || sec.antiflood){
     const key=chat+'|'+jidNum(sender);
     const now=Date.now();
     const st=spamState.get(key)||{times:[],last:''};
     
     st.times=st.times.filter(x=>now-x<Math.max(sec.spamWindow,sec.floodWindow));
     if(sec.antispam && t && st.last===t.toLowerCase()) st.times.push(now,now);
     else st.times.push(now);
     st.last=t.toLowerCase();
     ...
     if(flood||spam){
      try{await s.sendMessage(chat,{delete:m.key})}catch{}
      const n=addWarn(chat,sender,flood?'flood':'spam');
      if(n>=sec.warnLimit){try{await s.groupParticipantsUpdate(chat,[sender],'remove')}catch{}}
      return;
     }
    }
    ...
    if(reason || (sec.antimedia && mk)){
     try{await s.sendMessage(chat,{delete:m.key})}catch{}
     const why=reason||('media:'+mk);
     const n=addWarn(chat,sender,why);
   ...
   if(c==='setprefix'&&isOwner(m)){if(!a[0])return s.sendMessage(chat,{text:`Uso: ${p}setprefix !`});set('prefix',a[0]);return s.sendMessage(chat,{text:`Prefixo alterado para ${a[0]}`})}
   if(c==='setbotname'&&isOwner(m)){let n=arg(a);if(!n)return s.sendMessage(chat,{text:`Uso: ${p}setbotname 7kzz`});set('botName',n);return s.sendMessage(chat,{text:`Nome do bot alterado para: ${n}`})}
   if(c==='grupos'&&isOwner(m)){let gs=Object.entries(get().connectedGroups);return s.sendMessage(chat,{text:gs.length?gs.map(([id,n],i)=>`${i+1}. ${n}\n${id}`).join('\n\n'):'Nenhum grupo registrado.'})}
   if(c==='block'&&isOwner(m)){let n=a[0]?.replace(/\D/g,'');if(!n)return s.sendMessage(chat,{text:`Uso: ${p}block 5511999999999`});toggleArray('blocked',n,true);return s.sendMessage(chat,{text:'Usuário bloqueado.'})}
   if(c==='unblock'&&isOwner(m)){let n=a[0]?.replace(/\D/g,'');toggleArray('blocked',n,false);return s.sendMessage(chat,{text:'Usuário desbloqueado.'})}
   if(c==='broadcast'&&isOwner(m)){let msg=arg(a);if(!msg)return s.sendMessage(chat,{text:`Uso: ${p}broadcast mensagem`});for(const id of Object.keys(get().connectedGroups))await s.sendMessage(id,{text:msg});return s.sendMessage(chat,{text:`Atualização enviada para ${Object.keys(get().connectedGroups).length} grupos.`})}
   
   if(c==='oii'&&isOwner(m)){let d=get().oii;if(!d.text&&!d.imageUrl)return s.sendMessage(chat,{text:`Configure com ${p}oiimenu`});if(d.imageUrl){try{await s.sendMessage(chat,{image:{url:d.imageUrl},caption:d.text||d.title})}catch{await s.sendMessage(chat,{text:d.text||'Atualização'})}}else await s.sendMessage(chat,{text:d.text});return}
   if(c==='oiimenu'&&isOwner(m)){let sub=a[0]?.toLowerCase();if(!sub)return s.sendMessage(chat,{text:`${p}oiimenu texto <mensagem>\n${p}oiimenu foto <url>\n${p}oiimenu titulo <texto>\n${p}oiimenu descricao <texto>\n${p}oiimenu ver\n${p}oiimenu limpar`});if(sub==='texto'){set('oii',{...get().oii,text:arg(a,1)});return s.sendMessage(chat,{text:'Mensagem do oii salva.'})}if(sub==='foto'){set('oii',{...get().oii,imageUrl:a[1]||''});return s.sendMessage(chat,{text:'Foto do oii salva.'})}if(sub==='titulo'){set('oii',{...get().oii,title:arg(a,1)});return s.sendMessage(chat,{text:'Título salvo.'})}if(sub==='descricao'){set('oii',{...get().oii,description:arg(a,1)});return s.sendMessage(chat,{text:'Descrição salva.'})}if(sub==='limpar'){set('oii',{...get().oii,text:'',imageUrl:''});return s.sendMessage(chat,{text:'Configuração do oii limpa.'})}return s.sendMessage(chat,{text:JSON.stringify(get().oii,null,2)})}
   
   if(c==='addvip'&&isOwner(m)){let n=a[0]?.replace(/\D/g,'');toggleArray('vip',n,true);return s.sendMessage(chat,{text:'VIP adicionado.'})}
   if(c==='delvip'&&isOwner(m)){let n=a[0]?.replace(/\D/g,'');toggleArray('vip',n,false);return s.sendMessage(chat,{text:'VIP removido.'})}
   if(c==='listvip'&&isOwner(m))return s.sendMessage(chat,{text:get().vip.length?get().vip.join('\n'):'Nenhum VIP.'});
   ...
   if(c==='setname'){if(!chat.endsWith('@g.us')||!isOwner(m))return;await s.groupUpdateSubject(chat,arg(a));return s.sendMessage(chat,{text:'Nome do grupo alterado.'})}
   if(c==='setdesc'){if(!chat.endsWith('@g.us')||!isOwner(m))return;await s.groupUpdateDescription(chat,arg(a));return s.sendMessage(chat,{text:'Descrição alterada.'})}
   ...
   console.error(e);
   await s.sendMessage(chat,{text:'Erro ao executar o comando.'})
  }
 });
}
